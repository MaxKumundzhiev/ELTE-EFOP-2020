...
def generate_data(rows: int):
    ...
    user_role = ['admin', 'developer', 'customer']
...
def generate_graph(dataframe: pd.DataFrame):
    B = nx.Graph()
    B.add_nodes_from (dataframe['process_id'], bipartite=0)
    B.add_nodes_from (dataframe['process_type'], bipartite=1)
